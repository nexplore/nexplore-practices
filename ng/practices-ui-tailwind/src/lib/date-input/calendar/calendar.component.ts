...
import { PuiIconDatepickerTodayComponent } from '../../icons/icon-datepicker-today.component';
import { setHostAttr } from '../../util/utils';
import { PuiCalendarKeyboardNavigationService } from '../services/calendar-keyboard-navigation.service';
import {
    CalendarPeriod,
    CalendarPeriodItem,
    CalendarPeriodType,
    DateInput,
    PuiCalendarPeriodService,
...
import { PuiCalendarDateViewComponent } from './views/calendar-date-view.component';
import { PuiCalendarMonthViewComponent } from './views/calendar-month-view.component';
import { PuiCalendarYearViewComponent } from './views/calendar-year-view.component';
...
@Component({
    ...
    imports: [
        PuiCalendarDateViewComponent,
        PuiCalendarMonthViewComponent,
        PuiCalendarYearViewComponent,
    ...
    providers: [
        DestroyService,
        {
            ...
            useExisting: forwardRef(() => PuiCalendarComponent),
            multi: true,
        },
    ],
})
export class PuiCalendarComponent implements OnInit, ControlValueAccessor {
    ...
    private _calendarKeyboardNavigation = inject(PuiCalendarKeyboardNavigationService);
    private _datePeriodService = inject(PuiCalendarPeriodService);
    private _elementRef = inject<ElementRef<HTMLElement>>(ElementRef);
    private _translate = inject(TranslateService);

    /** Handler used by NG_VALUE_ACCESSOR */
    private _changeHandler?: (v: Date | undefined) => void;
    /** Handler used by NG_VALUE_ACCESSOR */
    private _touchedHandler?: () => void;

    private readonly _initialDateSignal = signal<Date | null>(null);
    private readonly _minDateSignal = signal<Date | null>(null);
    private readonly _maxDateSignal = signal<Date | null>(null);
    private readonly _selectionModeSignal = signal<CalendarPeriodType>('date');
    private readonly _requestedViewModeSignal = signal<CalendarViewMode | null>(null);
    private readonly _requestedPeriodSignal = signal<CalendarPeriod | null>(null);
    private readonly _periodSignal = computed(() => {
        ...
        const candidate = getSignalsFirstValue(
            this.focusedDateSignal(),
            this.selectedDateSignal(),
            this._initialDateSignal(),
            new Date(),
        ) as Date | null | undefined;
        const fallbackDate = this._restrictDateToMinMax(candidate ?? new Date());
    ...
    readonly selectedDateSignal = signal<Date | undefined>(undefined);
    ...
    readonly activeViewModeSignal = computed<CalendarViewMode>(() => {
        const vm = getSignalsFirstValue(
            (this._requestedViewModeSignal() ?? undefined) as CalendarViewMode | undefined,
            this._selectionModeSignal(),
            this.allowedViewModes[0],
        );
        return (vm ?? this.allowedViewModes[0]) as CalendarViewMode;
    });
    ...
    set initialDate(value: DateInput) {
        const parsed = this._datePeriodService.parse(this.activeViewModeSignal(), value, { strict: false }) ?? null;
        this._initialDateSignal.set(parsed);
    }
    ...
    set selectedDate(val: DateInput) {
        if (this.selectedDate !== val) {
            const parsed = this._datePeriodService.parse(this.selectionMode, val, { strict: false }) ?? undefined;
     ...
     */
    @Output()
    keyboardSubmit = new EventEmitter<Date | null>();
    ...
    ngOnInit(): void {
        ...
        if (!this.id) {
            ...
            this._setIdAttr('pui-calendar-' + nextUniqueId++);
        }
    }

    // NG_VALUE_ACCESSOR START
    writeValue(obj: Date | null | undefined): void {
        // Normalize undefined to null for external API
        this.selectedDate = (obj ?? null) as DateInput;
    }
    registerOnChange(fn: (v: Date | undefined) => void): void {
    ...
    onItemClick(item: CalendarPeriodItem) {
        ...
        if (this.activeViewModeSignal() === this.selectionMode) {
            ...
            setTimeout(() => {
                const sel = this.selectedDateSignal();
                if (sel) {
                    this.clickDate.emit(sel);
                }
            });
        } else {
    ...
    canChangePeriodView(nextView: CalendarViewMode) {
        const viewModes = this.allowedViewModes as ReadonlyArray<CalendarViewMode>;
        const currentIndex = viewModes.indexOf(nextView);
        if (currentIndex !== -1) {
            return true;
        } else {
            return false;
    ...
    isEventTargetOnCalendarOrCurrentDate(eventTarget: EventTarget | null) {
        if (!eventTarget) {
            return false;
        }
        return (
            eventTarget === this._elementRef.nativeElement ||
            (eventTarget instanceof HTMLElement && eventTarget.matches('[data-focused-date]'))
        );
    }

    isEventTargetOnCalendarOrAnySubchild(eventTarget: EventTarget | null) {
        if (!eventTarget) {
            return false;
        }
        return (
            eventTarget === this._elementRef.nativeElement ||
            (eventTarget instanceof HTMLElement && this._elementRef.nativeElement.contains(eventTarget))
        );
    }
    ...
    private _drillDownPeriodView(date: Date | null) {
        if (!date) {
            return;
        }
        let nextView: CalendarPeriodType;
        const active = this.activeViewModeSignal();
        const viewModes = this.allowedViewModes as ReadonlyArray<CalendarViewMode>;
        const currentIndex = viewModes.indexOf(active);
        if (currentIndex === 0) {
    ...
    private _canMoveToDate(date: Date | null | undefined) {
        if (!date) {
            return false;
        }
    ...
    private _setDateValue(date: Date | null | undefined) {
        if (date) {
    ...
    private _handleKeydown(event: KeyboardEvent) {
        if (event.code === 'Enter' && this.isEventTargetOnCalendarOrCurrentDate(event.target)) {
            if (this.activeViewModeSignal() === this.selectionMode) {
                const f = this.focusedDateSignal();
                if (f) {
                    this._setDateValue(f);
                    const sel = this.selectedDate;
                    if (sel) {
                        this._initialDateSignal.set(sel);
                        this.keyboardSubmit.emit(sel);
                    }
                }
            } else {

...
import { PuiIconDatepickerComponent } from '../icons/icon-datepicker.component';
...
import { PuiCalendarPeriodValidationService } from './services/calendar-period-validation.service';
import { CalendarPeriodType, DateInput, PuiCalendarPeriodService } from './services/calendar-period.service';

@Directive({
    standalone: true,
    selector: 'input[puiCalendarPickerInput]',
    providers: [
        DestroyService,
        Overlay,
        DatePipe,
        {
            ...
            multi: true,
            useExisting: forwardRef(() => PuiCalendarPickerInputDirective),
        },
        {
...
/** @internal */
export class PuiCalendarPickerInputDirective implements OnInit, ControlValueAccessor, Validator {
    ...
    protected readonly _calendarPeriodService = inject(PuiCalendarPeriodService);
    protected readonly _datepipe = inject(DatePipe);
    protected readonly _translate = inject(TranslateService);
    protected readonly _calendarPicker = inject(PuiCalendarPickerAnchorDirective);
    protected readonly _validationService = inject(PuiCalendarPeriodValidationService);
    ...
    ngOnInit(): void {
        ...
        this._formFieldService.setIcon({
            component: PuiIconDatepickerComponent,
            fill: true,
        ...
        const _iconClickSub = this._formFieldService.iconClick$.pipe(takeUntil(this._destroy$)).subscribe(() => {
            this._calendarPicker.openOverlay({ side: 'end', initiator: 'button' });
        });

        const _calendarDateChangeSub = this._calendarPicker.calendarDateChange
        ...
        const _calendarOpenChangeSub = this._calendarPicker.calendarOpenChange
            .pipe(takeUntil(this._destroy$))
            .subscribe((_open) => {
    ...
    writeValue(date: DateInput): void {
        ...
        this._setDatePickerValue(this._date ?? null);
        this._setInputValue(this._date ?? null);
    }

    registerOnChange(fn: (v: DateInput) => void): void {
    ...
    validate() {
        return (
            ...
            this._validationService.validateMinMax(this._date ?? undefined, {
                periodType: this.type,
    ...
    private _parseCurrentInputValue(): Date | null {
    ...
    private _setInputValue(date: Date | null) {
        if (!date) {
            this._elementRef.nativeElement.value = '';
            return;
        }
        const parsed = this._calendarPeriodService.parse(this.type, date, { strict: true });
        this._elementRef.nativeElement.value = this._calendarPeriodService.format(this.type, parsed) ?? '';
    }

    private _setDatePickerValue(date: Date | null) {
        this._calendarPicker.setDate(date);
    }

    private _setNewDate(newDate: Date | null) {
    ...
    private _onChangeNgModel(date: Date | null) {
        if (!this._onChangeHandler) {
            return;
        }
        const validDate = date && isValid(date) ? date : null;
        const modelValue = this.getModelValueHandler(validDate);

...
import { PuiDateInputDirective } from './date-input.directive';
import { PuiMonthInputDirective } from './month-input.directive';
import { CalendarPeriodType } from './services/calendar-period.service';
import { PuiYearInputDirective } from './year-input.directive';
...
const meta: Meta<Args> = {
    ...
    render: (args) => ({
        ...
        template: `
        <pui-form-field class="w-1/2 max-sm:w-full" [formGroup]="formGroup" [puiReadonly]="readonly" 
            ${
            ...
            >
            <label puiLabel>${args.label}</label>
            <input puiInput type="${args.type ?? 'date'}" 
...
export const WithNgModel: Story = {
    render: (args) => ({
        ...
        template: `
        <pui-form-field class="w-1/2 max-sm:w-full">
            <label puiLabel>With ngModel</label>
            <input puiInput type="date" [ngModel]="modelValue" (ngModelChange)="modelChange($event)" />
        </pui-form-field>

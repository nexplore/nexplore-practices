...
import { PuiSelectionMode } from './types';

@Directive({
    selector: '[puiSelectionMode], pui-table[selectionMode], pui-table[selectedItem], pui-table[selectedItems]',
    standalone: true,
})
export class PuiSelectionDirective implements OnDestroy {
    private _selectionSubject = new BehaviorSubject<any[]>([]);
    private _selectionModeSubject = new BehaviorSubject<PuiSelectionMode>('single');
    ...
    @Input()
    set puiSelectionMode(val: PuiSelectionMode) {
        this.selectionMode = val;
    }

    @Input()
    set selectionMode(val: PuiSelectionMode) {
    ...
    @Output()
    puiSelectionChange = this.selectionChange;
...
@Directive({
    selector: '[puiSelectable], pui-table-row[selectable]',
    standalone: true,
    providers: [DestroyService],
})
export class PuiSelectableDirective {
    private _selection = inject(PuiSelectionDirective);
    private _destroy$ = inject(DestroyService);

    private _selectableSubject = new BehaviorSubject<boolean | any>(false);
    @Input()
    set puiSelectable(val: boolean | any) {

...
const classNames =
    'cursor-pointer inline-block leading-normal no-underline outline-hidden hover:text-black after:block after:border-b-2';
...
const activeClassNames = 'after:scale-x-100 after:border-brand';
...
@Directive({
    standalone: true,
    selector: 'a[puiHeaderMainMenuItem], button[puiHeaderMainMenuItem]',
    providers: [DestroyService, MenuItemService],
})
export class PuiHeaderMainMenuItemDirective implements OnInit, OnDestroy {

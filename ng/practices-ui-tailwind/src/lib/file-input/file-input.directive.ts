...
import { PuiIconUploadComponent } from '../icons/icon-upload.component';
...
@Directive({
    standalone: true,
    selector: 'input[puiInput][type="file"]',
    providers: [
        DestroyService,
        {
            provide: NG_VALUE_ACCESSOR,
            multi: true,
            useExisting: forwardRef(() => PuiFileInputDirective),
        },
    ],
})
export class PuiFileInputDirective implements OnInit, ControlValueAccessor {
    ...
    ngOnInit() {
        ...
        this._formFieldService.setIcon({
            component: PuiIconUploadComponent,
            fill: true,
            clickable: true,
            invert: true,
        });

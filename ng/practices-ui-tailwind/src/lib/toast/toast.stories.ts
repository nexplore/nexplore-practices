...
import { PuiToastComponent } from './toast.component';
...
const meta: Meta<Args> = {
    title: 'PUIBE/toast',
    component: PuiToastComponent,
    tags: ['autodocs'],
    argTypes: {},
    decorators: [
        moduleMetadata({
            imports: [PuiToastComponent, BrowserAnimationsModule],
        }),
    ],
    render: (args) => ({
        props: {
        ...
        template: `
        <pui-toast [closeable]="closeable" [variant]="variant" [showAlertIcon]="showAlertIcon">
            ${args.text}
            <button puiToastAction *ngFor="let action of actions;">
                {{ action.text }}
            </button>
        </pui-toast>`,
    }),

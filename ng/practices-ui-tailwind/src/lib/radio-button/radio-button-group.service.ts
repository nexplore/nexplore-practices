...
export class RadioButtonGroupService implements OnDestroy {
    ...
    private readonly _nameSubject = new BehaviorSubject<string>(`pui-radio-button-group-${nextUniqueId++}`);
    private readonly _ngControlSubject = new BehaviorSubject<NgControl | null>(null);
    private _elementSubject = new BehaviorSubject<HTMLElement | null>(null);
    ...
    readonly isRequired$ = this._ngControl$.pipe(
        switchMap((ngControl) =>
            this.status$.pipe(map(() => ngControl.control?.hasValidator(Validators.required) ?? false)),
        ),
    );
    ...
    readonly errors$ = combineLatest([this._ngControl$, this.displayAsInvalid$]).pipe(
        map(([ngControl, displayAsInvalid]) => (displayAsInvalid ? ngControl.errors : null)),
    );

    readonly elementState$ = this._elementSubject.pipe(
        switchMap((el) => (el ? getElementFormStates$(el) : [])),

...
@Directive({
    standalone: true,
    selector: '[puiObserveScreenPosition]',
})
export class ObserveScreenPositionDirective implements OnInit, OnDestroy {
    private elementRef = inject(ElementRef<HTMLElement>);
    private isActive = true;
    private prevVal: ScreenPositionChange | null = null;

    /** Observes the position on the screen. Checks continously every frame */
    @Output()
    puiObserveScreenPosition = new EventEmitter<ScreenPositionChange>();

    @Input()
    set puiObserveScreenPositionEnabled(val: boolean) {
    ...
    private observe = (): void => {
        ...
        if (!isSame) {
            this.puiObserveScreenPosition.emit(change);
        }

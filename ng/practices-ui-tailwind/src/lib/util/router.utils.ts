...
export function getUrlForRoutes(routes: (PuiMetaRoute | PuiMetaRouteMatch)[]): string {
    function getPath(r: PuiMetaRoute<unknown> | PuiMetaRouteMatch): string {
        return (r as PuiMetaRouteMatch).pathWithAppliedParams ?? r.path;
    }
...
export type PuiMetaRouteMatch = PuiMetaRoute & {
...
export function getRoutesForUrl(
    ...
    function getAliasUrlMatch(route: PuiMetaRoute<unknown>): string | RegExp {
    ...
    function isRouteDataLinkMatch(route: PuiMetaRoute<unknown>, url: string): boolean {
    ...
    function getRouteMatchScore(route: PuiMetaRouteMatch | null): number {
        if (!route) {
            return 0;
        }
    ...
    function findBestMatchingRoute(
        routes: PuiMetaRoute[],
        params: { url: string; currentSegment?: string; exactMatch?: boolean; routePath?: PuiMetaRoute[] },
    ) {
    ...
    function findBestRouteAmongstMatches(allRoutes: PuiMetaRouteMatch[][]) {
        return allRoutes.reduce<{ longestRoutePath: PuiMetaRouteMatch[]; longestRouteScore: number }>(
    ...
    const recurse = (
        url: string,
        parentRoute: PuiMetaRoute,
        routePath: PuiMetaRouteMatch[],
        currentSegment?: string,
    ): PuiMetaRouteMatch[][] => {
        currentSegment = normalizeUrlForComparision(currentSegment ?? url, '');
        const routes: PuiMetaRouteMatch[] = getAllMatchingRoutes(

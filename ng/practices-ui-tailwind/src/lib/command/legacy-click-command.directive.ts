 ...
 */
@Directive({
    selector: '[puiLegacyClickCommand]',
    standalone: true,
    providers: [DestroyService],
})
export class PuiLegacyClickCommandDirective<TArgs, TResult> implements OnInit {
    @HostListener('click')
    onClick() {
     ...
     **/
    @Input()
    set puiClickCommand(v: '' | ILegacyCommand<TArgs, TResult>) {
        if (v !== '') {
            // This check for empty string is needed to support the existing syntax that goes: `<element puiClickCommand [clickCommand]="command">`
            this.command = v;
        }
    }
     ...
     * @deprecated
     *
     * Use `puiClickCommand` instead (from `@nexplore/practices-ng-commands`), or directly `clickCommand` on [puiButton] or [puiTeaser].
     **/
    @Input()

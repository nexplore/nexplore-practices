...
@Component({
    ...
    imports: [
        ...
        forwardRef(() => PuiTableColumnComponent),
    ],
})
export class PuiTableComponent implements Partial<TableViewSourceConfig<any>> {
    private _destroy$ = inject(DestroyService);
    private _elementRef = inject<ElementRef<HTMLElement>>(ElementRef);
    private _statusService = inject(StatusService);

    private _beforeColumnTemplate = new BehaviorSubject<ColumnTemplate | null>(null);
    private readonly _defaultTableColumnsSubject = new BehaviorSubject<Array<TableColumnItem<any>>>([]);
    private readonly _tableViewSourceSubject = new BehaviorSubject<TableViewSource<any> | null>(null);
    private readonly _interactionEventsSubject = new Subject<TableInteractionEvent>();
    private readonly _statusOptionsSubject = new BehaviorSubject<StatusProgressOptions | null>(null);

...
@Component({
    standalone: true,
    selector: 'pui-table-pagination-infinite-scroll',
    templateUrl: './table-pagination-infinite-scroll.component.html',
    imports: [AsyncPipe, TranslateModule, PuiIconSpinnerComponent],
    providers: [DestroyService],
})
export class PuiTablePaginationInfiniteScrollComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private _observer: IntersectionObserver | null = null; // will trigger reload on the end of the page
    ...
    ngOnInit() {
        ...
        const _pageSub = this._table.tableViewSource.page$.pipe(takeUntil(this._destroy$)).subscribe((page) => {
        ...
        const _intersectionSub = combineLatest([this._intersectionSubject, this.busySubject])
            .pipe(
    ...
    private loadNextPage() {
        ...
        const _prefetchSub = this._table.tableViewSource.prefetchNextPage(skip, take, isLastFetch).subscribe(() => {
            this.busySubject.next(false);
            this.busyCountSubject.next(this.busyCountSubject.value - 1);
        });
    }

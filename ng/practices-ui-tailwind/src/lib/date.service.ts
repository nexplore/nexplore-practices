...
export class DateService {
    ...
    // Initialize with a default to satisfy strictPropertyInitialization and ensure formatting works before any language event fires.
    locale: Locale = enUS;
    constructor() {
    ...
    parseOrDefault = (text: DateInput, defaultDate: Date) => this.parse(text) ?? defaultDate;

    parse(text: DateInput, referenceDate?: Date): Date | null {
    ...
    format(date: DateInput, dateFormat: DateFormat): string {
        const parsed = this.parse(date);
        if (!parsed) return '';
        return format(parsed, dateFormat.toString(), { locale: this.locale });
    }
    ...
    getDaysInMonth(dateOrYear: Date | number, month?: number) {
        const date = typeof dateOrYear === 'number' ? new Date(dateOrYear, month ?? 0) : dateOrYear;
        return getDaysInMonth(date);
    }

    getDayOfWeek(date: Date) {
        const val = this.format(date, DateFormat.WEEKDAY_NUMBER);
        return +val || 0;
    }
}

...
import { CalendarPeriodType, PuibeCalendarPeriodService } from './calendar-period.service';
...
export class PuibeCalendarPeriodValidationService {
    private readonly _calendarPeriodService = inject(PuibeCalendarPeriodService);

    validateMinMax(value: Date, config: { periodType: CalendarPeriodType; min?: Date; max?: Date }): ValidationErrors {
        if (value) {

...
export interface PuibeFlyoutConfig extends DialogConfig {
    /**
...
const DEFAULT_CONFIG: PuibeFlyoutConfig = {
...
export class PuibeFlyoutService {
    ...
    open<R = unknown>(
        ...
        const mergedConfig: PuibeFlyoutConfig = { ...DEFAULT_CONFIG, ...config, positionStrategy };

        this._dialogRef = this._dialog.open<R>(componentOrTemplateRef, mergedConfig);

        this._applyArrowtip(this._dialogRef);

        return this._dialogRef as DialogRef<R>;
    }
    ...
    private _applyArrowtip(dialogRef: DialogRef<unknown, unknown>) {
        const flyoutConfig = dialogRef.config as PuibeFlyoutConfig;

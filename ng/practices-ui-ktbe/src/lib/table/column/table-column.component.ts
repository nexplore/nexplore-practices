...
export class PuibeTableColumnComponent implements TableColumnItem<any>, AfterViewInit {
    ...
    set fieldName(val: string | number | symbol) {
        if (typeof val === 'object') {
            val = (val as TableColumnItem<any>).fieldName;
        }
    ...
    ngAfterViewInit(): void {
        ...
        this._column$.pipe(takeUntil(this._destroy$)).subscribe((column) => {
            setHostAttr('aria-sort', sortDirToAriaSort(column?.sortDir), this._elementRef);
        });
    }
    ...
    private _translateClickToSortLabel(column: TableColumnItem<any>): any {

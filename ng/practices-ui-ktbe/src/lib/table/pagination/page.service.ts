...
import { StateDebouncer } from './util';

// TODO: Refactor with signals, remove StateDebouncer
@Injectable()
export class PageService {
    private _stateDebouncer = new StateDebouncer();
    private _preventEmit = false;
    ...
    set size(size: number) {
        ...
        if (size !== oldSize) {
            if (!this._preventEmit) {
                this._stateDebouncer.changeStart();
            }
            this._size = size;
            if (size === 0) {
                this._current = 1;
            ...
            if (!this._preventEmit) {
                ...
                this._sizeChange.next(size);
                this._stateDebouncer.changeDone();
            }
        }
        this._preventEmit = false;
    }
